...
// ── Marks data ──
const MARKS_DATA = [
  { course: 'Tajweed & Nazrah', test: 'Quiz 1',          score: 92,  total: 100, grade: 'A+' },
  { course: 'Tajweed & Nazrah', test: 'Mid-Term Exam',   score: 85,  total: 100, grade: 'A'  },
  { course: 'Hadith Studies',   test: 'Quiz 1',          score: 78,  total: 100, grade: 'B'  },
  { course: 'Hadith Studies',   test: 'Assignment',      score: 88,  total: 100, grade: 'A'  },
  { course: 'Seerah',           test: 'Presentation',    score: 90,  total: 100, grade: 'A+' },
  { course: 'Seerah',           test: 'Written Test',    score: 74,  total: 100, grade: 'B'  },
  { course: 'Islamic Studies',  test: 'Final Exam',      score: 95,  total: 100, grade: 'A+' },
  { course: 'Fiqh',             test: 'Quiz 1',          score: 80,  total: 100, grade: 'A'  },
];

// ── Progress data ──
const PROGRESS_DATA = [
  { icon: '📖', course: 'Tajweed & Nazrah',  pct: 75, color: '',       milestones: 3, total: 4, items: ['Makhaarij','Noon Saakin Rules','Madd Rules','Final Assessment'] },
  { icon: '📜', course: 'Hadith Studies',    pct: 60, color: 'gold',   milestones: 3, total: 5, items: ['Arbaeen Nawawi','Riyad as-Salihin','Bulugh al-Maram','Sahih Bukhari','Final Exam'] },
  { icon: '🌟', course: 'Seerah',            pct: 80, color: 'rust',   milestones: 4, total: 5, items: ['Early Life','Prophethood','Hijrah','Madinah Period','Farewell Pilgrimage'] },
  { icon: '☪️', course: 'Islamic Studies',   pct: 50, color: 'navy',   milestones: 2, total: 4, items: ['Aqeedah Basics','Five Pillars','Akhlaq','Advanced Topics'] },
];
...
const ATTENDANCE_MAP = {
  1:'P', 2:'P', 3:'H', 4:'P', 5:'A',
  6:'P', 7:'P', 8:'P', 9:'P', 10:'A',
  11:'P', 12:'P', 13:'H', 14:'P', 15:'P',
  16:'P', 17:'A', 18:'P', 19:'P', 20:'P',
  21:'P', 22:'P', 23:'P', 24:'H', 25:'P',
...
function renderMarksTable() {
  ...
  MARKS_DATA.forEach(row => {
    ...
    if (pct >= 90) { gradeClass = 'grade-a-plus'; }
    else if (pct >= 75) { gradeClass = 'grade-a'; }
    else if (pct >= 60) { gradeClass = 'grade-b'; fillClass = 'gold-fill'; }
    else { gradeClass = 'grade-c'; fillClass = 'red-fill'; }
...
function renderProgressCards() {
  ...
  PROGRESS_DATA.forEach(d => {
    ...
    const milestonesHtml = Array.from({length: d.total}, (_, i) =>
    ...
    const itemsHtml = d.items.map((item, i) => {
      ...
      const label    = isDone ? '<span class="status-done">✓ Done</span>' : isGoing ? '<span class="status-going">⟳ In Progress</span>' : '<span class="status-todo">○ Upcoming</span>';
...
function renderCalendar() {
  ...
  if (pctEl) pctEl.textContent  = total ? Math.round((present/total)*100) + '%' : '—';
