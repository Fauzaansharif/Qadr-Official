...
(function () {
  ...
  function translateNavLinks(isUrdu) {
    document
      .querySelectorAll(".nav-links a, .mobile-nav a, .nav-brand-text span")
      .forEach((el) => {
        const original =
    ...
    // Nav brand subtitle
    const brandSubs = document.querySelectorAll(
      ".nav-brand-text span:last-child",
    );
    brandSubs.forEach((el) => {
      el.textContent = isUrdu ? "اسلامی اکادمی" : "E-Learning Academy";
  ...
  function translateNoticeBand(isUrdu) {
    ...
    if (isUrdu) {
      const base =
        band.querySelector("a") ?
          band.querySelector("a").getAttribute("href")
        : "#";
